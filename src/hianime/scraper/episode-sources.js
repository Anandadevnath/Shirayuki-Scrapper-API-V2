...
const HIANIME_EPISODES_URL = `${HIANIME_BASE_URL}/ajax/episodes/list`;
const HIANIME_SERVERS_URL = `${HIANIME_BASE_URL}/ajax/links/list`;
const HIANIME_LINKS_VIEW_URL = `${HIANIME_BASE_URL}/ajax/links/view`;
...
export const getHiAnimeEpisodeSources = async ({ animeEpisodeId, ep, server, category }) => {
  ...
  const { url, $ } = await fetchHiAnimePage(`/watch/${animeId}`, { ep: episodeNumber });
  ...
  if (!aniId) {
    throw new Error('HiAnime watch page did not include syncData anime_id');
  }

  // 2. Get episode list and find episode token
  const episodesEnc = await encKai(aniId);
  const episodesResp = await axios.get(HIANIME_EPISODES_URL, {
    proxy: false,
    timeout: 15000,
    headers: {
      ...ajaxHeaders,
  ...
  if (!episodesHtml) {
    throw new Error('HiAnime episodes ajax returned empty result');
  }
  ...
  const serversResp = await axios.get(HIANIME_SERVERS_URL, {
    proxy: false,
    timeout: 15000,
    headers: {
      ...ajaxHeaders,
  ...
  if (!serversHtml) {
    throw new Error('HiAnime servers ajax returned empty result');
  }
  ...
  const viewResp = await axios.get(HIANIME_LINKS_VIEW_URL, {
    proxy: false,
    timeout: 15000,
    headers: {
      ...ajaxHeaders,
  ...
  if (!encryptedEmbed) {
    throw new Error('HiAnime links/view returned empty result');
  }
  ...
  console.log('[getHiAnimeEpisodeSources] Decrypted Embed URL:', embedUrl);
  ...
  console.log('[getHiAnimeEpisodeSources] Video ID:', videoId);
  console.log('[getHiAnimeEpisodeSources] Embed Base:', embedBase);
  ...
  const resolveTracksForCategory = async (targetCategory) => {
    try {
      ...
      const altServersResp = await axios.get(HIANIME_SERVERS_URL, {
        proxy: false,
        timeout: 15000,
        headers: {
          ...ajaxHeaders,
      ...
      const altViewResp = await axios.get(HIANIME_LINKS_VIEW_URL, {
        proxy: false,
        timeout: 15000,
        headers: {
          ...ajaxHeaders,
      ...
      console.log('[getHiAnimeEpisodeSources] Alternate category track fetch failed:', err.message);
      return [];
    }
  };
  ...
  try {
    ...
    console.log('[getHiAnimeEpisodeSources] Fetching media from:', mediaUrl);
    ...
    try {
      ...
      if (typeof encryptedMedia === 'string' && encryptedMedia.includes('Just a moment')) {
        console.log('[getHiAnimeEpisodeSources] Cloudflare challenge detected; media response skipped.');
        encryptedMedia = null;
      }
    } catch (axiosError) {
      console.log('[getHiAnimeEpisodeSources] Axios request failed:', axiosError.message);
    }

    console.log('[getHiAnimeEpisodeSources] Encrypted media response:', encryptedMedia ? `length=${encryptedMedia.length}` : 'null');
    ...
    if (encryptedMedia && typeof encryptedMedia === 'string' && encryptedMedia.length > 0) {
      ...
      console.log('[getHiAnimeEpisodeSources] Full decrypted media data:', JSON.stringify(mediaData, null, 2));
      ...
      if (mediaData) {
        ...
        if (!tracks.length && normalizedCategory === 'sub') {
          ...
          if (fallbackTracks.length) {
            console.log('[getHiAnimeEpisodeSources] Using dub category tracks as fallback for sub request.');
        ...
        console.log('[getHiAnimeEpisodeSources] m3u8:', m3u8);
        ...
        if (m3u8) {
          if (!tracks.length) {
            try {
              ...
              console.log('[getHiAnimeEpisodeSources] Parsing m3u8 content for tracks...');
              ...
              console.log('[getHiAnimeEpisodeSources] Parsed tracks:', JSON.stringify(tracks));
            } catch (m3u8Error) {
              console.log('[getHiAnimeEpisodeSources] Failed to fetch/parse m3u8:', m3u8Error.message);
            }
          }
    ...
    console.error('[getHiAnimeEpisodeSources] Error fetching/decrypting media:', mediaError.message);
  }
  ...
  console.log('[getHiAnimeEpisodeSources] No m3u8 found, returning embed URL');
  ...
  console.log('[getHiAnimeEpisodeSources] Fallback tracks from embedData:', JSON.stringify(fallbackTracks));
